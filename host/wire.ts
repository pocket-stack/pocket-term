// host/wire.ts — the host side of the SVC WIRE (PKNT) protocol:
// framing, the device hello, and the discovery beacon. Byte layouts come from
// contracts/spec/spec.ts "SVC WIRE protocol" and must stay bit-identical to
// engine/core/src/wire.rs and hosts/3ds/src/svcwire.c. Pure functions — the
// sockets live in serve.ts.
...
export { WIRE_BEACON_PORT, WIRE_MSG, WIRE_PORT } from "../vendor/pocketjs/contracts/spec/spec.ts";
...
export function encodeCtrl(line: string): Uint8Array {
  return encodeFrame(WIRE_MSG.ctrl, new TextEncoder().encode(line));
}
